...
describe("GET /api/admin/usage/logs?format=csv", () => {
  ...
  it("returns csv attachment with escaped values", async () => {
    ...
    expect(body).toContain("id,time,user,model,type,prompt_tokens,completion_tokens,total_tokens,stream,duration_ms,status,backend_id,backend_url");
    expect(body).toContain('"success","a0b1c2d3-e4f5-4678-9012-abcdefabcdef","http://gpu-1.internal:8000/v1"');
